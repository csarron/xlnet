...
def configure_tpu(FLAGS):
    ...
    if FLAGS.use_tpu:
        strategy = None
        logger.info('Use TPU without distribute strategy.')
        ...
        logger.info('Use MirroredStrategy with %d devices.',
...
def my_init_from_checkpoint(init_checkpoint, to_be_init_vars=None,
                            replace_map=None):
    """Compute the union of the current variables and checkpoint variables."""
    if to_be_init_vars is None:
        to_be_init_vars = tf.trainable_variables()
    ...
    for init_var in init_vars:
        ...
        for old_name in replace_map or {}:
            new_init_name = new_init_name.replace(old_name,
                                                  replace_map[old_name])
...
def get_assignment_map_from_checkpoint(tvars, init_checkpoint):
    ...
    for x in init_vars:
        (name, var) = (x[0], x[1])
        # logger.info('original name: %s', name)
    ...
    def _do_use_weight_decay(self, param_name):
        ...
        if self.exclude_from_weight_decay:
            for r in self.exclude_from_weight_decay:
                if re.search(r, param_name) is not None:
                    logger.info('Adam WD excludes {}'.format(param_name))
                    return False
        return True
...
if __name__ == "__main__":
    ...
    flags.DEFINE_string("clean_output_model_dir", "",
                        "output dir for cleaned ckpt")
