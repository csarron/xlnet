    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            if len(line) <= a_column:
                logger.warning('Incomplete line, ignored.')
                continue
    ...
    def _create_examples(self, input_file):
        ...
        with tf.gfile.Open(input_file) as f:
            ...
            for i, line in enumerate(reader):
                ...
                examples.append(
                    InputExample(guid=str(i), text_a=text_a, text_b=None,
                                 label=label))
        return examples
...
def file_based_convert_examples_to_features(
    ...
    if tf.gfile.Exists(output_file) and not FLAGS.overwrite_data:
        logger.info("Do not overwrite tfrecord {} exists.".format(output_file))
        return

    logger.info("Create new tfrecord {}.".format(output_file))
    ...
    for (ex_index, example) in enumerate(examples):
        if ex_index % 10000 == 0:
            logger.info("Writing example {} of {}".format(ex_index,
                                                          len(examples)))
    ...
    def model_fn(features, labels, mode, params):
        ...
        if FLAGS.is_regression:
            (total_loss, per_example_loss, logits
             ) = function_builder.get_regression_loss(FLAGS, features,
                                                      is_training)
        else:
            (total_loss, per_example_loss, logits
...
def main(_):
    ...
    if FLAGS.do_train:
        ...
        logger.info("Use tfrecord file {}".format(train_file))

        train_examples = processor.get_train_examples(FLAGS.data_dir)
        np.random.shuffle(train_examples)
        logger.info("Num of train samples: {}".format(len(train_examples)))
    ...
    if FLAGS.do_eval:
        ...
        for filename in filenames:
            if filename.endswith(".index"):
                ...
                logger.info("Add {} to eval list.".format(cur_filename))
    ...
    if FLAGS.do_predict:
        ...
        with tf.gfile.Open(os.path.join(predict_dir, "{}.tsv".format(
            ...
            for pred_cnt, result in enumerate(estimator.predict(
                ...
                if pred_cnt % 1000 == 0:
                    logger.info("Predicting submission for example: {}".format(
                        pred_cnt))
