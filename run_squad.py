...
def convert_examples_to_features(examples, sp_model, max_seq_length,
    ...
    for (example_index, example) in enumerate(examples):
        ...
        if all(v is None for v in orig_to_chartok_index) or f[
            N - 1, M - 1] < 0.8 * N:
...
class FeatureWriter(object):
    ...
    def process_feature(self, feature):
        ...
        if self.is_training:
            features["answer_start"] = create_int_feature(
    ...
    for (example_index, example) in enumerate(all_examples):
        ...
        for pred in prelim_predictions:
            ...
            final_text = paragraph_text[
                         start_orig_pos: end_orig_pos + 1].strip()
    ...
    def input_fn(params):
        ...
        if num_hosts > 1:
            ...
            if num_files >= num_hosts:
                ...
                my_end_file_id = min((host_id + 1) * num_files_per_host,
                                     num_files)
                input_paths = global_input_paths[
                              my_start_file_id: my_end_file_id]
...
def main(_):
    ...
    if FLAGS.do_predict:
        ...
        eval_input_fn = input_fn_builder(
            ...
            input_glob=FLAGS.eval_record_file,
            seq_length=FLAGS.max_seq_length,
            is_training=False,
        ...
        for result in estimator.predict(input_fn=eval_input_fn,
            ...
            cur_results.append((unique_id, start_logits, end_logits))
