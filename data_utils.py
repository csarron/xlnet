...
from prepro_utils import encode_ids
...
def _create_data(idx, input_paths):
    ...
    for input_path in input_paths:
        ...
        for line in tf.gfile.Open(input_path):
            if line_cnt % 100000 == 0:
                logger.info("Loading line %d", line_cnt)
            line_cnt += 1
        ...
        logger.info("Finish with line %d", line_cnt)
        if line_cnt == 0:
            continue
    ...
    logger.info("[Task %d] Total number line: %d", idx, total_line_cnt)
    ...
    logger.info("Using perm indices %s for pass %d",
...
def create_data(_):
    ...
    logger.info("Use glob: %s", FLAGS.input_glob)
    logger.info("Find %d files: %s", len(file_paths), file_paths)

    task_file_paths = file_paths[FLAGS.task::FLAGS.num_task]
    if not task_file_paths:
        logger.info("Exit: task %d has no file to process.", FLAGS.task)
        return

    logger.info("Task %d process %d files: %s",
...
def _split_a_and_b(data, sent_ids, begin_idx, tot_len, extend_target=False):
    ...
    if begin_idx + tot_len >= data_len:
        logger.info("[_split_a_and_b] returns None: "
...
def _sample_mask(sp, seg, reverse=False, max_gram=5, goal_num_predict=None):
    ...
    while cur_len < seg_len:
        ...
        beg = cur_len + l_ctx
        while beg < seg_len and not _is_start_piece(
            sp.IdToPiece(seg[beg].item())):
            beg += 1
        if beg >= seg_len:
            break
...
def create_tfrecords(save_dir, basename, data, bsz_per_host, seq_len,
    ...
    logger.info("Start writing %s.", save_path)
    ...
    while i + seq_len <= data_len:
        if num_batch % 500 == 0:
            logger.info("Processing batch %d", num_batch)
        ...
        for idx in range(bsz_per_host):
            ...
            results = _split_a_and_b(
                ...
                extend_target=True)
            if results is None:
                logger.info("Break out with seq idx %d", i)
                all_ok = False
                break
            ...
            tgt = np.concatenate(
                [tgt, a_target, b_target, cls_array, cls_array])
    ...
    record_writer.close()
    logger.info("Done writing %s. Num of batches: %d", save_path, num_batch)
...
def parse_files_to_dataset(parser, file_names, split, num_batch, num_hosts,
    ...
    logger.info("Host %d handles %d files", host_id, len(file_paths))
...
def get_input_fn(
    ...
    logger.info("Use the following tfrecord dirs: %s", tfrecord_dirs)

    for idx, record_dir in enumerate(tfrecord_dirs):
        record_glob = os.path.join(record_dir, record_glob_base)
        logger.info("[%d] Record glob: %s", idx, record_glob)

        record_paths = sorted(tf.gfile.Glob(record_glob))
        logger.info("[%d] Num of record info path: %d",
        ...
        for record_info_path in record_paths:
            if num_passes is not None:
                ...
                if len(fields) == 5 and pass_id >= num_passes:
                    logger.info("Skip pass %d: %s", pass_id, record_info_name)
                    continue
            ...
            with tf.gfile.Open(record_info_path, "r") as fp:
                ...
                if num_passes is not None:
                    ...
                    cur_record_info["num_batch"] += int(
                        info["num_batch"] * ratio)
        ...
        logger.info("[Dir %d] Number of chosen batches: %s",
                    idx, cur_record_info["num_batch"])
        logger.info("[Dir %d] Number of chosen files: %s",
                    idx, len(cur_record_info["filenames"]))
        logger.info(cur_record_info["filenames"])

        # add `cur_record_info` to global `record_info`
        record_info["num_batch"] += cur_record_info["num_batch"]
        record_info["filenames"] += cur_record_info["filenames"]

    logger.info("Total number of batches: %d",
                record_info["num_batch"])
    logger.info("Total number of files: %d",
                len(record_info["filenames"]))
    logger.info(record_info["filenames"])
